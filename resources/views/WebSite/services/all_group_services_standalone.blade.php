...
<head>
    ...
    <style>
        :root {
            /* ألوان أساسية بسيطة لهذه الصفحة المستقلة */
            --page-primary-color: #2E7D32; /* أخضر داكن */
            --page-secondary-color: #FFAB00; /* كهرماني */
            --page-text-color: #333;
        ...
        *, *::before, *::after { box-sizing: inherit; }
        ...
        .group-service-card .group-notes { font-size: 0.9rem; color: #555; margin-bottom: 1rem; line-height: 1.7; min-height: 3.4em; /* Space for approx 2 lines */ display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; }
        .group-service-card .services-list-title { font-family: var(--page-font-heading); font-size: 1rem; font-weight: 600; color: var(--page-heading-color); margin-bottom: 0.5rem; margin-top: 1rem; padding-top: 0.75rem; border-top: 1px dashed var(--page-border-color); }
        .group-service-card .services-list { list-style: none; padding: 0; margin: 0; font-size: 0.85rem; }
        .group-service-card .services-list li { padding: 0.25rem 0; color: #444; display: flex; align-items: center; }
        ...
        .group-service-card .btn-view-details { background-color: var(--page-primary-color); color: white; border: none; padding: 0.5rem 1.25rem; font-size: 0.9rem; font-weight: 600; border-radius: 50px; text-decoration: none; transition: background-color 0.2s ease; }
    ...
    <header class="page-header-minimal">
        <div class="container-fluid">
            ...
            <div class="nav-links-minimal">
                <a href="{{ route('website.services.all') }}"><i class="fas fa-medkit"></i> الخدمات الفردية</a>
                ...
                <div class="col-lg-4 col-md-6 col-sm-12">
                    <div class="group-service-card">
                        <div class="card-header-block">
                            ...
                            {{-- عرض السعر الإجمالي للمجموعة (Total_with_tax) --}}
                        ...
                        <div class="card-body-block">
                            @if($group->notes)
                                <p class="group-notes">{{ Str::limit(strip_tags($group->notes), 120) }}</p>
                            @endif

                            {{-- التحقق من وجود العلاقة وأنها ليست null وأنها Collection قبل التكرار --}}
                            @if($group->service_group && $group->service_group instanceof \Illuminate\Support\Collection && $group->service_group->isNotEmpty())
                                ...
                                <ul class="services-list">
                                    @foreach($group->service_group->take(4) as $service) {{-- عرض أول 4 خدمات --}}
                                        <li><i class="fas fa-check"></i> {{ $service->name }}
                                            @if(isset($service->pivot) && isset($service->pivot->quantity) && $service->pivot->quantity > 1)
                                                (الكمية: {{ $service->pivot->quantity }})
                                            @endif
                                        </li>
                                    @endforeach
                                    @if($group->service_group->count() > 4)
                                        <li><i class="fas fa-ellipsis-h"></i> وخدمات أخرى...</li>
                                    @endif
                                </ul>
                            @else
                                <p class="text-muted small">لا توجد تفاصيل للخدمات المتضمنة في هذه الباقة حالياً.</p>
                            @endif
                        </div>
                        <div class="card-footer-block">
                            {{-- يمكنك تغيير الرابط # ليوجه لصفحة تفاصيل الباقة إذا وجدت أو صفحة حجز --}}
