...
<head>
    ...
    <style>
        :root {
            ...
            --deptpage-primary-color: {{ $settings['deptpage_primary_color'] ?? '#1E88E5' }}; /* A strong, professional blue */
            --deptpage-primary-color-rgb: {{ implode(',', sscanf($settings['deptpage_primary_color'] ?? '#1E88E5', '#%02x%02x%02x')) ?? '30, 136, 229' }};
            --deptpage-secondary-color: {{ $settings['deptpage_secondary_color'] ?? '#43A047' }}; /* A complementary green */
            --deptpage-text-color: #37474F; /* Darker text for readability */
            --deptpage-heading-color: #263238; /* Even darker for headings */
            --deptpage-light-text: #78909C;
            --deptpage-bg-color: #ECEFF1; /* Very light blue-gray background */
            --deptpage-card-bg: #ffffff;
            --deptpage-border-color: #CFD8DC; /* Softer border */
            --deptpage-radius: 12px; /* More rounded corners */
            --deptpage-shadow: 0 10px 30px rgba(0, 0, 0, 0.08);
            --deptpage-font-primary: 'Cairo', sans-serif; /* Cairo for main text for a softer feel */
            --deptpage-font-secondary: 'Tajawal', sans-serif; /* Tajawal for headings for sharpness */
        }

        html, body { margin: 0; padding: 0; box-sizing: border-box; font-family: var(--deptpage-font-primary); background-color: var(--deptpage-bg-color); color: var(--deptpage-text-color); line-height: 1.7; font-size: 16px; }
        *, *::before, *::after { box-sizing: inherit; }

        .departments-standalone-container { max-width: 1240px; margin: 0 auto; padding: 2.5rem 1rem; }

        /* Minimalist Header for Standalone Page */
        .standalone-dept-header { background-color: var(--deptpage-card-bg); padding: 1rem 0; box-shadow: 0 3px 10px rgba(0,0,0,0.05); margin-bottom: 2.5rem; }
        .standalone-dept-header .container-fluid { display: flex; justify-content: space-between; align-items: center; }
        .standalone-dept-header .site-branding .site-title { font-family: var(--deptpage-font-secondary); font-size: 1.7rem; color: var(--deptpage-primary-color); margin: 0; font-weight: 800; text-decoration: none; }
        .standalone-dept-header .back-to-home a { color: var(--deptpage-primary-color); text-decoration: none; font-weight: 600; font-size: 0.95rem; }
        .standalone-dept-header .back-to-home a i { margin-inline-end: 0.4rem; }
        .standalone-dept-header .back-to-home a:hover { text-decoration: underline; }
        ...
        .department-card-standalone .icon-wrapper-standalone {
            padding: 2.5rem 1.5rem 1.5rem; /* More padding for icon */
            text-align: center;
        ...
        .department-card-standalone .icon-wrapper-standalone .department-icon {
            ...
            color: var(--deptpage-card-bg); /* Icon color white */
            margin-bottom: 0.5rem; /* Space below icon if text is added */
            line-height: 1;
            display: inline-block;
        ...
        .department-card-standalone .department-name-standalone a {
            font-family: var(--deptpage-font-secondary);
            font-size: 1.5rem; /* 24px, larger name */
            font-weight: 700;
            color: var(--deptpage-heading-color);
            margin-bottom: 0.75rem;
            display: block;
        ...
        .department-card-standalone .btn-details-standalone i {
            margin-inline-start: 0.5rem; /* Arrow after text */
        ...
        /* No Results Styling */
        .no-departments-standalone { text-align: center; padding: 3rem 1rem; background-color: var(--deptpage-card-bg); border-radius: var(--deptpage-radius); box-shadow: var(--deptpage-shadow); }
        .no-departments-standalone .no-dept-icon { font-size: 4rem; color: var(--deptpage-primary-color); opacity: 0.6; margin-bottom: 1rem; }
        .no-departments-standalone h3 { font-family: var(--deptpage-font-secondary); font-size: 1.6rem; color: var(--deptpage-heading-color); margin-bottom: 0.75rem; }
        .no-departments-standalone p { font-size: 1rem; color: var(--deptpage-light-text); }

        /* Pagination Styling */
        .pagination-standalone .page-item .page-link { color: var(--deptpage-primary-color); border-radius: var(--deptpage-radius); font-weight: 500; }
        .pagination-standalone .page-item.active .page-link { background-color: var(--deptpage-primary-color); border-color: var(--deptpage-primary-color); color: var(--deptpage-card-bg); }
        .pagination-standalone .page-item.disabled .page-link { color: var(--deptpage-light-text); }

        /* Footer Styling */
        .standalone-dept-footer { text-align: center; padding: 1.5rem 0; margin-top: 3rem; background-color: var(--deptpage-card-bg); border-top: 1px solid var(--deptpage-border-color); font-size: 0.9rem; color: var(--deptpage-light-text); }
    ...
    <main class="departments-standalone-container">
        <section class="departments-page-title-standalone">
            ...
            <p class="title-description-standalone animate__animated animate__fadeInUp" data-wow-delay="0.3s">
                اكتشف مجموعة واسعة من الأقسام الطبية التي نقدمها، والمصممة لتلبية كافة احتياجاتك الصحية بأعلى معايير الجودة.
            </p>
        </section>

        <div class="row g-4"> {{-- Bootstrap 5 row with gutters --}}
            @forelse ($sections as $section)
                <div class="col-lg-4 col-md-6 col-sm-12 animate__animated animate__fadeInUp" data-wow-delay="{{ ($loop->index % 3) * 0.1 + 0.4 }}s">
                    <div class="department-card-standalone">
                        <div class="icon-wrapper-standalone">
                            <span class="department-icon {{ $section->icon_class ?? 'fas fa-clinic-medical' }}"></span> {{-- Default icon if not provided --}}
                        </div>
                        <div class="content-wrapper-standalone">
                            <h3 class="department-name-standalone">
                                <a href="{{ route('website.department.details', $section->id) }}" title="تفاصيل قسم {{ $section->name }}">
                                    {{ $section->name }}
                                </a>
                            </h3>
                            ...
                            @endif
                            <a href="{{ route('website.department.details', $section->id) }}" class="btn-details-standalone">
                                اقرأ المزيد <i class="fas {{ LaravelLocalization::getCurrentLocaleDirection() == 'rtl' ? 'fa-arrow-left' : 'fa-arrow-right' }}"></i>
                            </a>
                        </div>
                    </div>
                </div>
