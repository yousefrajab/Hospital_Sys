{{-- resources/views/Dashboard/laboratorie_employee/edit.blade.php --}}
@extends('Dashboard.layouts.master')

@php
    $employeeName = $laboratorie_employee->name ?? 'موظف غير مسمى';
@endphp
...
@section('page-header')
    <div class="breadcrumb-header justify-content-between">
        ...
        <div class="d-flex my-xl-auto right-content">
            <a href="{{ route('admin.laboratorie_employee.index') }}" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="row justify-content-center mt-4">
        <div class="col-lg-10 col-md-12">
            <div class="card-3d p-lg-6 p-4"> {{-- استخدام بطاقة 3D مع padding --}}
                ...
                <form action="{{ route('admin.laboratorie_employee.update', $laboratorie_employee->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 d-flex flex-column align-items-center">
                             <div class="avatar-upload mb-4">
                                ...
                                <img id="lab_photo_preview_{{ $laboratorie_employee->id }}"
                                     src="{{ $laboratorie_employee->image ? URL::asset('Dashboard/img/laboratorieEmployees/' . $laboratorie_employee->image->filename) : URL::asset('Dashboard/img/default_avatar.png') }}"
                                     alt="صورة الموظف">
                                <label for="lab_photo_input_{{ $laboratorie_employee->id }}">
                                    <i class="fas fa-camera"></i>
                                </label>
                                <input id="lab_photo_input_{{ $laboratorie_employee->id }}" type="file" accept="image/*,image/webp" name="photo"
                                       onchange="loadFile(event, 'lab_photo_preview_{{ $laboratorie_employee->id }}')"> {{-- استخدام دالة loadFile --}}
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="text" id="name_edit_{{ $laboratorie_employee->id }}" name="name" class="form-control-modern @error('name') is-invalid @enderror" value="{{ old('name', $laboratorie_employee->name) }}" required placeholder=" "> {{-- placeholder مهم لـ floating label --}}
                                        <label for="name_edit_{{ $laboratorie_employee->id }}">الاسم الكامل <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="text" id="national_id_edit_{{ $laboratorie_employee->id }}" name="national_id" class="form-control-modern @error('national_id') is-invalid @enderror" value="{{ old('national_id', $laboratorie_employee->national_id) }}" pattern="[0-9]{9,10}" maxlength="10" oninput="this.value = this.value.replace(/[^0-9]/g, '')" required placeholder=" ">
                                        <label for="national_id_edit_{{ $laboratorie_employee->id }}">رقم الهوية <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="email" id="email_edit_{{ $laboratorie_employee->id }}" name="email" class="form-control-modern @error('email') is-invalid @enderror" value="{{ old('email', $laboratorie_employee->email) }}" required placeholder=" ">
                                        <label for="email_edit_{{ $laboratorie_employee->id }}">البريد الإلكتروني <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="tel" id="phone_edit_{{ $laboratorie_employee->id }}" name="phone" class="form-control-modern @error('phone') is-invalid @enderror" value="{{ old('phone', $laboratorie_employee->phone) }}" pattern="^05\d{8}$" maxlength="10" required placeholder=" ">
                                        <label for="phone_edit_{{ $laboratorie_employee->id }}">رقم الهاتف <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <select name="status" id="status_edit_{{ $laboratorie_employee->id }}" class="form-select-modern w-100 @error('status') is-invalid @enderror" required>
                                            <option value="1" {{ old('status', $laboratorie_employee->status) == 1 ? 'selected' : '' }}>نشط</option>
                                            <option value="0" {{ old('status', $laboratorie_employee->status) == 0 ? 'selected' : '' }}>غير نشط</option>
                                        </select>
                                        <label for="status_edit_{{ $laboratorie_employee->id }}">الحالة <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label position-relative">
                                        <input type="password" id="password_edit_{{ $laboratorie_employee->id }}" name="password" class="form-control-modern password-input @error('password') is-invalid @enderror" autocomplete="new-password" minlength="8" placeholder=" ">
                                        <label for="password_edit_{{ $laboratorie_employee->id }}">كلمة المرور الجديدة</label>
                                        <button type="button" class="toggle-password-eye" onclick="togglePasswordVisibility('password_edit_{{ $laboratorie_employee->id }}')">
                    ...
                    <div class="d-flex justify-content-end mt-5 gap-4"> {{-- استخدام d-flex و gap --}}
                        <a href="{{ route('admin.laboratorie_employee.index') }}" class="btn-custom btn-cancel">
