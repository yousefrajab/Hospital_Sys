...
@section('title', 'إنشاء خدمة طبية جديدة')
...
@section('css')
    ...
    <style>
        ...
        .card-header.card-header-form { /* Specific style for form card headers */
        ...
        /* Enhanced Floating Label & Input Group Styling */
        .form-group.floating-label-group { position: relative; margin-bottom: 2rem; } /* Increased margin */
        ...
        .form-group.floating-label-group .form-control.floating-textarea {
            ...
            padding: 1.25rem .9rem .6rem; /* Top, Horizontal, Bottom */
            line-height: 1.5;
            border-radius: .45rem; /* Slightly more rounded inputs */
            border: 1px solid var(--bs-border-color);
            background-clip: padding-box; /* Important for select arrows */
        ...
        .form-group.floating-label-group .floating-label {
            position: absolute;
            top: 0;
            left: 0; right: auto; /* For RTL */
            height: 100%;
            padding: .9rem .9rem; /* Match input padding */
            pointer-events: none;
            border: 1px solid transparent;
            transform-origin: 0 0;
            transition: opacity .15s ease-in-out,transform .15s ease-in-out;
            color: #858796; /* Lighter label text */
            font-size: 0.95rem;
            opacity: .75;
            display: flex; /* For icon alignment */
            align-items: center; /* For icon alignment */
        }
        .form-group.floating-label-group .floating-input:not(:placeholder-shown) ~ .floating-label, /* Uses placeholder=" " trick */
        ...
        .form-group.floating-label-group .floating-textarea:focus ~ .floating-label {
            opacity: .75;
            transform: scale(.80) translateY(-.8rem) translateX(.2rem); /* Adjusted float */
         ...
         .form-group.floating-label-group .floating-textarea:focus {
            ...
            background-color: #fff; /* Ensure background is white on focus */
        }
        ...
        .form-group.floating-label-group .input-group .floating-label { left: auto; right: 45px; /* Adjust based on prepend width + padding */ }
        ...
        .form-switch.form-switch-lg .form-check-input {
            width: 2.5em; height: 1.25em; margin-right: 5em;
        ...
        <div class="my-auto">
            <div class="d-flex align-items-center">
                 <h4 class="content-title mb-0 my-auto">
                    <i class="fas fa-plus-square fa-lg text-primary me-2"></i>
                    <span class="fw-bold">إنشاء خدمة طبية جديدة</span>
                </h4>
            </div>
             <p class="text-muted mt-1 mb-0 tx-13">قم بإدخال تفاصيل الخدمة التي ستقدمها لمرضاك.</p>
        </div>
...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-lg-9 col-xl-8"> {{-- Control form width --}}
            <div class="card" data-aos="fade-up" data-aos-delay="200">
                <div class="card-header card-header-form">
                    <h4 class="card-title-enhanced"><i class="fas fa-notes-medical"></i>تفاصيل الخدمة الجديدة</h4>
                </div>
                <div class="card-body p-4 p-lg-5">
                    <form action="{{ route('doctor.services_management.store') }}" method="post" class="needs-validation" novalidate autocomplete="off">
                        @csrf
                        ...
                        <div class="form-group floating-label-group" data-aos="fade-up" data-aos-delay="400">
                             <div class="input-group">
                                ...
                                <input type="number" name="price" id="service_price" class="form-control floating-input @error('price') is-invalid @enderror"
                                   value="{{ old('price') }}" placeholder=" " min="0" step="0.01" required>
                        ...
                        <div class="form-group floating-label-group" data-aos="fade-up" data-aos-delay="500">
                            <textarea class="form-control floating-textarea @error('description') is-invalid @enderror" name="description" id="service_description"
                                      rows="4" placeholder=" ">{{ old('description') }}</textarea>
                        ...
                        <div class="form-group" data-aos="fade-up" data-aos-delay="600">
                            <label class="form-label fw-bold text-gray-700 d-block mb-2">حالة الخدمة <span class="text-danger">*</span></label>
                            <div class="form-check form-switch form-switch-lg">
                                <input type="hidden" name="status" value="0">
                                <input type="checkbox" class="form-check-input @error('status') is-invalid @enderror" id="service_status"
                                       name="status" value="1" role="switch" {{ old('status', 1) ? 'checked' : '' }}>
                                <label class="form-check-label" for="service_status" id="service_status_label">
                                    {{ old('status', 1) ? trans('doctors.Enabled') : trans('doctors.Not_enabled') }}
                                </label>
                                @error('status')
...
@section('js')
    {{-- لا نحتاج Select2 JS إذا ألغينا قسم الخدمات العامة --}}
    ...
    <script>
        $(document).ready(function() {
            ...
            // Bootstrap 5 form validation (standard script)
            (function () {
              'use strict'
            ...
            // Update status label text and color on change
            const statusCheckbox = document.getElementById('service_status');
            const statusLabel = document.getElementById('service_status_label');

            function updateStatusLabel() {
                if (statusCheckbox.checked) {
                    statusLabel.textContent = "{{ trans('doctors.Enabled') }}";
                    statusLabel.classList.remove('text-danger-emphasis'); // Use BS5 emphasis class
                    statusLabel.classList.add('text-success-emphasis');
                } else {
                    statusLabel.textContent = "{{ trans('doctors.Not_enabled') }}";
                    statusLabel.classList.remove('text-success-emphasis');
                    statusLabel.classList.add('text-danger-emphasis');
                }
            }
            if (statusCheckbox) {
                statusCheckbox.addEventListener('change', updateStatusLabel);
                updateStatusLabel(); // Initial state
            }
