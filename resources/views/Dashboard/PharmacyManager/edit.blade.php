{{-- resources/views/Dashboard/pharmacy_manager/edit.blade.php --}}
@extends('Dashboard.layouts.master')

@php
    $employeeName = $pharmacy_manager->name ?? 'موظف غير مسمى';
@endphp
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                <h4 class="content-title mb-0 my-auto">مديري الصيدلية</h4>
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ تعديل بيانات / {{ $employeeName }}</span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            <a href="{{ route('admin.pharmacy_manager.index') }}" class="btn btn-sm btn-outline-secondary">
            ...
            <div class="card-3d p-lg-6 p-4"> {{-- استخدام بطاقة 3D مع padding --}}
                <div class="d-flex justify-content-between align-items-center mb-5">
                    <h3 class="section-title mb-0">
                        <i class="fas fa-flask me-2"></i> تعديل بيانات مدير الصيدلية
                    </h3>
                </div>

                <form action="{{ route('admin.pharmacy_manager.update', $pharmacy_manager->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 d-flex flex-column align-items-center">
                             <div class="avatar-upload mb-4">
                                ...
                                <img id="lab_photo_preview_{{ $pharmacy_manager->id }}"
                                     src="{{ $pharmacy_manager->image ? URL::asset('Dashboard/img/pharmacy_managers/' . $pharmacy_manager->image->filename) : URL::asset('Dashboard/img/default_avatar.png') }}"
                                     alt="صورة الموظف">
                                <label for="lab_photo_input_{{ $pharmacy_manager->id }}">
                                    <i class="fas fa-camera"></i>
                                </label>
                                <input id="lab_photo_input_{{ $pharmacy_manager->id }}" type="file" accept="image/*,image/webp" name="photo"
                                       onchange="loadFile(event, 'lab_photo_preview_{{ $pharmacy_manager->id }}')"> {{-- استخدام دالة loadFile --}}
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="text" id="name_edit_{{ $pharmacy_manager->id }}" name="name" class="form-control-modern @error('name') is-invalid @enderror" value="{{ old('name', $pharmacy_manager->name) }}" required placeholder=" "> {{-- placeholder مهم لـ floating label --}}
                                        <label for="name_edit_{{ $pharmacy_manager->id }}">الاسم الكامل <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="text" id="national_id_edit_{{ $pharmacy_manager->id }}" name="national_id" class="form-control-modern @error('national_id') is-invalid @enderror" value="{{ old('national_id', $pharmacy_manager->national_id) }}" pattern="[0-9]{9,10}" maxlength="10" oninput="this.value = this.value.replace(/[^0-9]/g, '')" required placeholder=" ">
                                        <label for="national_id_edit_{{ $pharmacy_manager->id }}">رقم الهوية <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="email" id="email_edit_{{ $pharmacy_manager->id }}" name="email" class="form-control-modern @error('email') is-invalid @enderror" value="{{ old('email', $pharmacy_manager->email) }}" required placeholder=" ">
                                        <label for="email_edit_{{ $pharmacy_manager->id }}">البريد الإلكتروني <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <input type="tel" id="phone_edit_{{ $pharmacy_manager->id }}" name="phone" class="form-control-modern @error('phone') is-invalid @enderror" value="{{ old('phone', $pharmacy_manager->phone) }}" pattern="^05\d{8}$" maxlength="10" required placeholder=" ">
                                        <label for="phone_edit_{{ $pharmacy_manager->id }}">رقم الهاتف <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label">
                                        <select name="status" id="status_edit_{{ $pharmacy_manager->id }}" class="form-select-modern w-100 @error('status') is-invalid @enderror" required>
                                            <option value="1" {{ old('status', $pharmacy_manager->status) == 1 ? 'selected' : '' }}>نشط</option>
                                            <option value="0" {{ old('status', $pharmacy_manager->status) == 0 ? 'selected' : '' }}>غير نشط</option>
                                        </select>
                                        <label for="status_edit_{{ $pharmacy_manager->id }}">الحالة <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="floating-label position-relative">
                                        <input type="password" id="password_edit_{{ $pharmacy_manager->id }}" name="password" class="form-control-modern password-input @error('password') is-invalid @enderror" autocomplete="new-password" minlength="8" placeholder=" ">
                                        <label for="password_edit_{{ $pharmacy_manager->id }}">كلمة المرور الجديدة</label>
                                        <button type="button" class="toggle-password-eye" onclick="togglePasswordVisibility('password_edit_{{ $pharmacy_manager->id }}')">
                    ...
                    <div class="d-flex justify-content-end mt-5 gap-4"> {{-- استخدام d-flex و gap --}}
                        <a href="{{ route('admin.pharmacy_manager.index') }}" class="btn-custom btn-cancel">
