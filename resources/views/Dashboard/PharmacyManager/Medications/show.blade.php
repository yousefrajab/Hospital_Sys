...
@section('css')
    ...
    <style>
        :root {
            ...
            --admin-light: #f8f9fa;
            --admin-dark: #212529;
            --admin-body-bg: #f4f6f9; /* لون خلفية الصفحة */
            ...
            --admin-shadow-lg: 0 0.5rem 1.5rem rgba(0, 0, 0, 0.1); /* ظل أكثر بروزاً */
        ...
        @media (prefers-color-scheme: dark) {
            :root {
                ...
                --admin-light: #2d3748; /* تعديل للداكن */
            }
        }
        ...
        .details-card {
            ...
            margin-bottom: 2rem;
            overflow: hidden; /* مهم للـ header */
        }
        ...
        .details-card-header {
            ...
            align-items: center;
            border-bottom: 1px solid transparent; /* لا حاجة للحد */
        }
        .details-card-header h3 {
            margin-bottom: 0;
            font-weight: 600;
            font-size: 1.5rem; /* عنوان أكبر */
            display: flex;
            align-items: center;
        }
        .details-card-header h3 i {
            margin-inline-end: 0.75rem;
            font-size: 1.8rem; /* أيقونة أكبر للعنوان */
            opacity: 0.9;
        }
        .status-badge {
            font-size: 0.9rem;
        ...
        .section-title {
            ...
            display: inline-block; /* ليأخذ الـ border-bottom عرض النص فقط */
        }
        .section-title i { margin-inline-end: 0.5rem; color: var(--admin-primary); }

        .info-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); /* شبكة مرنة */
            gap: 1.25rem; /* مسافات أكبر */
        ...
        .info-item:hover { transform: translateY(-3px); box-shadow: var(--admin-shadow); }
        .info-item-label {
            display: flex;
            align-items: center;
            font-size: 0.85rem;
        ...
        .info-item-label i {
            ...
            width: 18px; /* تحديد عرض للأيقونة لتناسق المحاذاة */
            text-align: center;
        }
        .info-item-value {
            font-size: 1rem;
            font-weight: 600;
            color: var(--admin-dark);
            word-break: break-word; /* لكسر النصوص الطويلة */
        }

        .stock-table { margin-top: 0.5rem; }
        .stock-table th {
            background-color: var(--admin-light) !important; /* تعديل مهم */
            color: var(--admin-dark);
            font-weight: 600;
            font-size: 0.9rem;
            text-align: center;
        ...
        /* تلوين صفوف المخزون */
        .stock-table .table-danger, .stock-table .table-danger > th, .stock-table .table-danger > td { background-color: #f8d7da !important; color: #721c24 !important; }
        .stock-table .table-warning, .stock-table .table-warning > th, .stock-table .table-warning > td { background-color: #fff3cd !important; color: #856404 !important; }
        @media (prefers-color-scheme: dark) {
            .stock-table th { background-color: #374151 !important; }
            .stock-table .table-danger, .stock-table .table-danger > th, .stock-table .table-danger > td { background-color: #4b2d30 !important; color: #f8d7da !important; }
            .stock-table .table-warning, .stock-table .table-warning > th, .stock-table .table-warning > td { background-color: #4d4123 !important; color: #fff3cd !important; }
        }

        .btn-action-group { display: flex; gap: 0.75rem; }
        .btn-icon-text .fas, .btn-icon-text .far { margin-inline-end: 0.35rem; }

        .alert-custom-info {
            background-color: var(--admin-info-bg, #e0f7fa); /* لون أزرق فاتح جداً */
        ...
        @media (prefers-color-scheme: dark) {
            .alert-custom-info {
                background-color: #2c5282; color: #bee3f8; border-color: #4299e1;
            }
        }
        .no-stock-message i { font-size: 2.5rem; margin-bottom: 0.75rem; }
        ...
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <span class="text-muted mt-0 tx-13">/ تفاصيل الدواء: <strong>{{ $medicationNameForTitle }}</strong></span>
                </div>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content btn-action-group">
            <a href="{{ route('pharmacy_manager.medications.edit', $medication->id) }}" class="btn btn-outline-warning btn-sm btn-icon-text">
                <i class="fas fa-edit"></i> تعديل
            </a>
            <a href="{{ route('pharmacy_manager.medications.index') }}" class="btn btn-outline-secondary btn-sm btn-icon-text">
                ...
                <div class="details-card-body">
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            ...
                            <strong class="info-item-value">{{ $medication->created_at->translatedFormat('d F Y, H:i') }}</strong>
                        </div>
                    ...
                    @if($medication->relationLoaded('stocks') && $medication->stocks->where('quantity_on_hand', '>', 0)->count() > 0)
                        <div class="table-responsive shadow-sm" style="border-radius: var(--admin-radius-md); overflow: hidden;">
                            <table class="table table-sm table-bordered stock-table mb-0">
                                ...
                                <tbody>
                                    @foreach($medication->stocks->where('quantity_on_hand', '>', 0)->sortBy('expiry_date') as $index => $stock)
                                        <tr class="{{ $stock->expiry_date->isPast() ? 'table-danger' : ($stock->expiry_date->isBefore(now()->addMonths(config('pharmacy.expiry_warning_months', 3))) ? 'table-warning' : '') }}">
                                            ...
                                            <td>
                                                ...
                                                @elseif($stock->expiry_date->isBefore(now()->addMonths(config('pharmacy.expiry_warning_months', 3))))
                                                    <i class="fas fa-exclamation-circle text-warning ms-1" title="قارب على الانتهاء"></i>
                                                @endif
                                            </td>
                        ...
                        <small class="form-text text-muted mt-2 d-block">
                            ...
                            <span class="badge bg-warning-soft ms-2 me-1"> </span> قارب على الانتهاء (خلال {{ config('pharmacy.expiry_warning_months', 3) }} أشهر)
                        </small>
                    @else
                    ...
                    <div class="text-center mt-4">
                        {{-- يجب أن يكون لديك route لإدارة المخزون --}}
                        {{-- <a href="{{ route('pharmacy_manager.stocks.index', ['medication_id' => $medication->id]) }}" class="btn btn-primary btn-sm btn-icon-text">
                            <i class="fas fa-tasks"></i> إدارة دفعات المخزون
                        </a> --}}
                        <a href="{{ route('pharmacy_manager.medications.stocks.create', ['medication_id' => $medication->id]) }}" class="btn btn-success btn-sm btn-icon-text ms-2">
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            // NotifIt messages
            @if (session('success'))
                notif({msg: "<i class='fas fa-check-circle me-2'></i> {{ session('success') }}", type: "success", position: "bottom-right", autohide: true, timeout: 5000});
            @endif
            @if (session('error'))
                notif({msg: "<i class='fas fa-exclamation-triangle me-2'></i> {{ session('error') }}", type: "error", position: "bottom-right", autohide: true, timeout: 7000});
            @endif
        });
    </script>
@endsection
