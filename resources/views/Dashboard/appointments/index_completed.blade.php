...
{{-- ========================== CSS Section ========================== --}}
@section('css')
    {{-- استيراد نفس مكتبات CSS المستخدمة في الصفحات الأخرى --}}
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css"/>
    {{-- Notify CSS (اختياري إذا احتجت إشعارات هنا) --}}
    {{-- <link href="{{URL::asset('Dashboard/plugins/notify/css/notifIt.css')}}" rel="stylesheet"/> --}}
     {{-- Flatpickr CSS (للفلترة) --}}
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
    <link rel="stylesheet" href="https://npmcdn.com/flatpickr/dist/l10n/ar.css">

    {{-- نفس أنماط CSS المستخدمة في index و index2 مع تعديل بسيط للبطاقة المنتهية --}}
    <style>
        :root { /* لوحة الألوان */
            ...
            --text-dark: #34495e; --text-muted: #95a5a6; --text-completed: #6c757d; /* لون للحالة المنتهية */
        ...
        .page-title i { color: var(--text-completed); } /* تغيير لون أيقونة العنوان */

        /* تصميم البطاقة */
        .admin-appointments-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.5rem; }
        .admin-appointment-card { background: var(--white-color); border-radius: 12px; box-shadow: var(--card-shadow); border-left: 5px solid var(--text-completed); /* خط رمادي للمنتهي */ padding: 1.25rem; display: flex; flex-direction: column; opacity: 0.85; /* جعلها باهتة قليلاً */ transition: all 0.3s ease; }
        ...
        .appointment-time .info-value { font-weight: 600; color: var(--text-completed); } /* لون مختلف للوقت المنتهي */

        /* الأزرار (عادة لا توجد أزرار للمنتهي) */
        ...
        .status-badge-completed { background-color: rgba(108, 117, 125, 0.1); color: #5a6268; font-size: 0.8rem; font-weight: 600; padding: 0.4rem 0.8rem; border-radius: 50px; display: inline-flex; align-items: center; gap: 0.3rem; }
        ...
        .no-appointments-admin i { font-size: 3rem; display: block; margin-bottom: 1rem; opacity: 0.5; color: var(--text-muted); }
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                <h4 class="content-title mb-0 my-auto">المواعيد</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ المواعيد المنتهية</span>
            </div>
        </div>
         <div class="d-flex my-xl-auto right-content">
             ...
             <a href="{{ route('admin.cancelled') }}" class="btn btn-outline-danger btn-sm"><i class="fas fa-times me-1"></i> الملغاة</a>
        </div>
    </div>
    <!-- breadcrumb -->
@endsection
...
@section('content')
    ...
    <div class="admin-appointments-container">
        <div class="page-title-container">
            <h1 class="page-title"><i class="fas fa-history"></i> المواعيد المنتهية</h1>
        </div>

        {{-- يمكن إضافة قسم فلترة هنا أيضاً إذا لزم الأمر --}}
            ...
            @forelse ($appointments as $appointment)
                <div class="admin-appointment-card animate__animated animate__fadeInUp" data-status="{{ $appointment->type }}" style="animation-delay: {{ $loop->index * 0.05 }}s;">
                    <div class="card-main-info">
                        ...
                        {{-- وقت الموعد المنتهي --}}
                        <div class="info-row appointment-time">
                            <span class="info-icon"><i class="fas fa-calendar-check"></i></span>
                            <div class="info-details">
                                <span class="info-label">تاريخ ووقت الموعد</span>
                                <span class="info-value">{{ $appointment->appointment ? $appointment->appointment->translatedFormat('l، d M Y - h:i A') : '-' }}</span>
                            </div>
                        </div>
                        {{-- الهاتف (اختياري) --}}
                        <div class="info-row">
                    ...
                    {{-- فوتر البطاقة (لا يوجد إجراءات عادة) --}}
                    <div class="card-actions">
                        <span class="status-badge-completed"><i class="fas fa-check-double me-1"></i> منتهي</span>
                        {{-- يمكنك إضافة زر لعرض تفاصيل الفاتورة أو السجل الطبي المرتبط --}}
                        {{-- <a href="#" class="btn btn-sm btn-outline-info">عرض السجل</a> --}}
                    </div>
                </div>
                {{-- لا حاجة لمودالات هنا عادةً --}}
            @empty
                 <div class="no-appointments-admin">
                      <i class="fas fa-calendar-check"></i>
                     <p>لا توجد مواعيد منتهية لعرضها حالياً.</p>
                 </div>
            @endforelse
        </div>
     ...
     <script>
         document.addEventListener('DOMContentLoaded', function() {
             ...
             if(document.getElementById('dateFilter')) {
              ...
              console.log('Completed appointments page loaded.');
         });
     </script>
@endsection
