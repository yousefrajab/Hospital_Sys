...
class UpdatePharmacyManagerRequest extends FormRequest
{
    /**
    ...
    {
        $employeeId = $this->route('pharmacy_manager'); // إذا كان اسم البارامتر في الـ route هو 'pharmacy_manager'
        ...
        $currentPharmacyManager = $employeeId ? PharmacyManager::find($employeeId) : null;
        ...
        return [
            ...
            //     'string',
            //     'max:20',
            //     Rule::unique('pharmacy_managers', 'national_id')->ignore($employeeId),
            // ],
            'national_id' => [
                'required',
                'string',
                'digits:9',
                Rule::unique('pharmacy_managers', 'national_id')->ignore($employeeId), // 1. فريد في جدول pharmacy_managers (باستثناء الحالي)
                function ($attribute, $value, $fail) use ($currentPharmacyManager) { // ** استخدام $currentPharmacyManager هنا **
                    // تحقق من global_national_ids فقط إذا تغير الإيميل عن الإيميل الأصلي للطبيب
                    if (strtolower($value) !== strtolower($currentPharmacyManager->getOriginal('national_id'))) {
            ...
            'email' => [
                'required',
                'email',
                Rule::unique('pharmacy_managers', 'email')->ignore($employeeId),
                function ($attribute, $value, $fail) use ($currentPharmacyManager) {
                    ...
                    if ($currentPharmacyManager && strtolower($value) !== strtolower($currentPharmacyManager->getOriginal('email'))) {
            ...
            'phone' => [
                ...
                'string',
                'regex:/^05\d{8}$/',
                Rule::unique('pharmacy_managers', 'phone')->ignore($employeeId),
            ],
