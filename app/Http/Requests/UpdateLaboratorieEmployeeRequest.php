    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'national_id' => [
                ...
                'string',
                'digits:9',
                Rule::unique('laboratorie_employees', 'national_id')->ignore($employeeId), // 1. فريد في جدول laboratorie_employees (باستثناء الحالي)
                function ($attribute, $value, $fail) use ($currentLaboratorieEmployee) { // ** استخدام $currentLaboratorieEmployee هنا **
                    // تحقق من global_national_ids فقط إذا تغير الإيميل عن الإيميل الأصلي للطبيب
                    if (strtolower($value) !== strtolower($currentLaboratorieEmployee->getOriginal('national_id'))) {
