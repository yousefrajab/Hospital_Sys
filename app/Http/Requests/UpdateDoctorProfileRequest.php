...
class UpdateDoctorProfileRequest extends FormRequest
{
    ...
    {
        $currentDoctor = Auth::guard('doctor')->user(); // ** الحصول على الموديل الحالي للطبيب **
        ...
        return [
            ...
            'email' => [
                'required',
                'email',
                Rule::unique('doctors', 'email')->ignore($doctorId), // 1. فريد في جدول doctors (باستثناء الحالي)
                function ($attribute, $value, $fail) use ($currentDoctor) { // ** استخدام $currentDoctor هنا **
                    // تحقق من global_emails فقط إذا تغير الإيميل عن الإيميل الأصلي للطبيب
                    if (strtolower($value) !== strtolower($currentDoctor->getOriginal('email'))) {
