...
namespace App\Http\Requests\Dashboard\PharmacyEmployee; // تم تعديل المسار قليلاً
...
class UpdatePharmacyEmployeeProfileRequest extends FormRequest
{
    public function authorize(): bool
    {
        // التأكد من أن المستخدم المسجل هو موظف أشعة
        return Auth::guard('pharmacy_employee')->check();
    }

    public function rules(): array
    {
        $currentPatient = Auth::guard('pharmacy_employee')->user(); // ** الحصول على الموديل الحالي للمريض **
        ...
        return [
            'name' => ['required', 'string', 'max:255'],
             'national_id' => [
                ...
                Rule::unique('pharmacy_employees', 'national_id')->ignore($employeeId), // 1. فريد في جدول pharmacy_employees
            ...
            'email' => [
                ...
                Rule::unique('pharmacy_employees', 'email')->ignore($employeeId), // 1. فريد في جدول pharmacy_employees
