...
namespace App\Http\Requests\Dashboard\PharmacyManager; // تم تعديل المسار قليلاً
...
class UpdatePharmacyManagerProfileRequest extends FormRequest
{
    public function authorize(): bool
    {
        // التأكد من أن المستخدم المسجل هو موظف أشعة
        return Auth::guard('pharmacy_manager')->check();
    }

    public function rules(): array
    {
        $currentPatient = Auth::guard('pharmacy_manager')->user(); // ** الحصول على الموديل الحالي للمريض **
        ...
        return [
            'name' => ['required', 'string', 'max:255'],
             'national_id' => [
                ...
                Rule::unique('pharmacy_managers', 'national_id')->ignore($employeeId), // 1. فريد في جدول pharmacy_managers
            ...
            'email' => [
                ...
                Rule::unique('pharmacy_managers', 'email')->ignore($employeeId), // 1. فريد في جدول pharmacy_managers
