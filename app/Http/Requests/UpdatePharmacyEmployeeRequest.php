...
class UpdatePharmacyEmployeeRequest extends FormRequest
{
    /**
    ...
    {
        $employeeId = $this->route('pharmacy_employee'); // إذا كان اسم البارامتر في الـ route هو 'pharmacy_employee'
        ...
        $currentPharmacyEmployee = $employeeId ? PharmacyEmployee::find($employeeId) : null;
        ...
        return [
            ...
            //     'string',
            //     'max:20',
            //     Rule::unique('pharmacy_employees', 'national_id')->ignore($employeeId),
            // ],
            'national_id' => [
                'required',
                'string',
                'digits:9',
                Rule::unique('pharmacy_employees', 'national_id')->ignore($employeeId), // 1. فريد في جدول pharmacy_employees (باستثناء الحالي)
                function ($attribute, $value, $fail) use ($currentPharmacyEmployee) { // ** استخدام $currentPharmacyEmployee هنا **
                    // تحقق من global_national_ids فقط إذا تغير الإيميل عن الإيميل الأصلي للطبيب
                    if (strtolower($value) !== strtolower($currentPharmacyEmployee->getOriginal('national_id'))) {
            ...
            'email' => [
                'required',
                'email',
                Rule::unique('pharmacy_employees', 'email')->ignore($employeeId),
                function ($attribute, $value, $fail) use ($currentPharmacyEmployee) {
                    ...
                    if ($currentPharmacyEmployee && strtolower($value) !== strtolower($currentPharmacyEmployee->getOriginal('email'))) {
            ...
            'phone' => [
                ...
                'string',
                'regex:/^05\d{8}$/',
                Rule::unique('pharmacy_employees', 'phone')->ignore($employeeId),
            ],
