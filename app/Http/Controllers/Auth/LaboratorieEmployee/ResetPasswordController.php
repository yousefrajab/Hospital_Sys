...
class ResetPasswordController extends Controller // ** تأكد أن اسم الكلاس هو ResetPasswordController **
{
    ...
    {
        $this->middleware('guest:laboratorie_employee');
        $this->redirectTo = route('dashboard.laboratorie_employee'); // أو RouteServiceProvider::ADMIN إذا كان معرفًا
    }

    public function showResetForm(Request $request, $token = null)
    {
        // resources/views/Dashboard/auth/laboratorie_employee/passwords/reset.blade.php
        return view('Dashboard.auth.laboratorie_employee.passwords.reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        // استخدام الـ broker 'laboratorie_employees'
        $response = Password::broker('laboratorie_employees')->reset(
        ...
        if ($response == Password::PASSWORD_RESET) {
            ...
            // Auth::guard('laboratorie_employee')->login($user); // $user هو المستخدم الذي تم تحديثه
