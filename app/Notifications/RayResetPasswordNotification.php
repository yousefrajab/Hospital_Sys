...
class RayResetPasswordNotification extends Notification // لا تحتاج لـ ShouldQueue افتراضيًا هنا
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        $resetUrl = url(route('ray_employee.password.reset', [
            'token' => $this->token,
        ...
        return (new MailMessage)
            ->subject(Lang::get('إعادة تعيين كلمة المرور لحساب موظف الأشعة')) // عنوان الإيميل
            ->greeting(Lang::get('مرحباً ') . $notifiable->name . '!') // تحية شخصية
            ->line(Lang::get('لقد تلقيت هذا البريد الإلكتروني لأننا تلقينا طلب إعادة تعيين كلمة المرور لحساب  موظف الأشعة الخاص بك.'))
