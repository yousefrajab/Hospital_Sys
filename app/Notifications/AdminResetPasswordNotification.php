...
class AdminResetPasswordNotification extends Notification // لا تحتاج لـ ShouldQueue افتراضيًا هنا
{
    use Queueable;
     ...
     *
     * @param  mixed  $notifiable  (هذا هو كائن موديل Admin)
    ...
    public function toMail($notifiable)
    {
        ...
        // 'admin.password.reset' هو اسم الـ route الذي عرفناه سابقًا
        // نمرر الـ token والإيميل الخاص بالمستخدم (notifiable)
        $resetUrl = url(route('admin.password.reset', [
            'token' => $this->token,
            'email' => $notifiable->getEmailForPasswordReset(), // دالة من موديل Admin (CanResetPassword)
        ], false)); // false لعدم تضمين التوقيع (signed URL) إذا لم تكن تستخدمه

        return (new MailMessage)
            ->subject(Lang::get('إعادة تعيين كلمة المرور لحساب مدير النظام')) // عنوان الإيميل
            ->greeting(Lang::get('مرحباً ') . $notifiable->name . '!') // تحية شخصية
            ->line(Lang::get('لقد تلقيت هذا البريد الإلكتروني لأننا تلقينا طلب إعادة تعيين كلمة المرور لحساب مدير النظام الخاص بك.'))
            ->action(Lang::get('إعادة تعيين كلمة المرور'), $resetUrl) // زر الإجراء
            ->line(Lang::get('رابط إعادة تعيين كلمة المرور هذا سينتهي صلاحيته بعد :count دقائق.', ['count' => config('auth.passwords.admins.expire')])) // مدة صلاحية الرمز
