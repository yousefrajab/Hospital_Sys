...
class LabResetPasswordNotification extends Notification // لا تحتاج لـ ShouldQueue افتراضيًا هنا
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        // 'patient.password.reset' هو اسم الـ route الذي عرفناه سابقًا
        // نمرر الـ token والإيميل الخاص بالمستخدم (notifiable)
        $resetUrl = url(route('laboratorie_employee.password.reset', [
            'token' => $this->token,
        ...
        return (new MailMessage)
            ->subject(Lang::get('إعادة تعيين كلمة المرور لحساب موظف المختبر')) // عنوان الإيميل
            ->greeting(Lang::get('مرحباً ') . $notifiable->name . '!') // تحية شخصية
            ->line(Lang::get('لقد تلقيت هذا البريد الإلكتروني لأننا تلقينا طلب إعادة تعيين كلمة المرور لحساب  موظف المختبر الخاص بك.'))
            ->action(Lang::get('إعادة تعيين كلمة المرور'), $resetUrl) // زر الإجراء
            ->line(Lang::get('رابط إعادة تعيين كلمة المرور هذا سينتهي صلاحيته بعد :count دقائق.', ['count' => config('auth.passwords.patients.expire')])) // مدة صلاحية الرمز
